...
const DASHBOARD_SCREENS: Screen[] = ["dashboard", "learning", "assessment", "submission", "certificate"];
...
function parseStoredList(value: string | undefined, fallback: string[]) {
...
function parseBreakdown(value: string | undefined) {
  const fallback = {
    functionality: 28,
    code_quality: 18,
    security: 13,
    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "backdrop-blur-xl bg-[#060616]/80 border-b border-white/5" : ""}`}>
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <button className="text-sm text-white/60 hover:text-white transition-colors px-4 py-2" onClick={() => onNavigate("dashboard")}>
            Sign In
          </button>
          <MagButton
      ...
      <section className="py-24 px-6 max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          <div className="relative rounded-3xl overflow-hidden p-12 md:p-16 text-center bg-gradient-to-br from-violet-600/20 via-blue-600/10 to-transparent border border-violet-500/20">
            ...
            <MagButton onClick={() => onNavigate("dashboard")}
...
function SubmissionPage({ runtime }: { runtime: WorkismRuntime }) {
  const { user, setUser, setEvaluation, onNavigate } = runtime;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const activeUser = user ?? await ensureDemoUser();
      setUser(activeUser);
      setStatus("Submitting project...");
      const submission = await submitPythonProject(activeUser.id, repo.trim(), branch);
...
export default function App() {
  ...
  const renderDashboardContent = () => {
    switch (screen) {
      case "dashboard": return <DashboardHome onNavigate={setScreen} />;
      case "skills": return <SkillsPage onNavigate={setScreen} />;
      case "learning": return <LearningPage onNavigate={setScreen} />;
      case "assessment": return <AssessmentPage onNavigate={setScreen} />;
      ...
      default: return <DashboardHome onNavigate={setScreen} />;
    }
  };
  ...
  return (
    <>
      <CursorGlow />
      <DashboardLayout screen={screen} onNavigate={setScreen} user={user} onLogout={handleLogout}>
